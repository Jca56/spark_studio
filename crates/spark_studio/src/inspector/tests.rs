...
pub(super) fn inside(outer: Viewport, inner: Viewport, what: &str) {
    assert!(
        inner.x >= outer.x - 0.5
            && inner.y >= outer.y - 0.5
...
pub(super) fn draw(e: &mut Editor, tool: Tool, from: [f32; 2], to: [f32; 2]) -> usize {
    e.choose_tool(tool);
...
pub(super) fn page(e: &Editor, scale: f32, scroll: f32) -> Page {
